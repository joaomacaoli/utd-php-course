...
SELECT 
	...
	--SUM(`product_price`*`item_quantity`) AS `VALOR_TOTAL`
FROM
	`orders` INNER JOIN `clients` -- INNER JOIN `users` INNER JOIN `products` INNER JOIN `items` 
	ON `orders`.`client_id` = `clients`.`id_client` 
	--ON `orders`.`user_id` = `users`.`id_user` AND
	--ON `items`.`order_id` = `orders`.`id_order`	 AND 
	-- ON `items`.`product_id` = `products`.`id_product`
GROUP BY 
	`orders`.`id_order`;	
...
FROM
	...
	INNER JOIN `items` ON `items`.`order_id` = `orders`.`id_order`
	INNER JOIN `products` ON `items`.`product_id` = `products`.`id_product`
GROUP BY 
	`orders`.`id_order`;	
...
UPDATE `clients` SET `client_name` = 'CARLA RAFAELA TELES', `client_adddress` = 'RUA BEM ALÍ, 223'
WHERE `id_client` = 1;
...
-- DELETAR UM REGISTRO
DELETE FROM `products` WHERE `id_product`= 11;
